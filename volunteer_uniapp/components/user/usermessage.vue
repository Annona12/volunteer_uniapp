<template>
	<view>
		<view class="header status" >
			...
			<view class="setting"><image src="../../static/components/user/完善个人信息.png"></image></view>
		</view>
		<view class="volunteer_message">
			<view class="box">
				<view class="label" v-for="(row,index) in orderTypeLise" :key="row.name" hover-class="hover" @tap="toOrderType(index)">
					<view class="icon"><view class="badge" v-if="row.badge>0">{{row.badge}}</view><image :src="'../../static/components/user/'+row.icon"></image></view>
					{{row.name}}
				</view>
			</view>
		</view> 
		<view class="list" v-for="(list,list_i) in severList" :key="list_i">
			<view class="li" v-for="(li,li_i) in list" @tap="toPage(list_i,li_i)" v-bind:class="{'noborder':li_i==list.length-1}" hover-class="hover" :key="li.name" >
				<view class="icon"><image ></image></view>
...
<script>
	export default {
		data() {
			return {
				userinfo:{
					face:'../../static/common/volunteer.jpg',
					username:'游客',
					integral:0   
		 },
				orderTypeLise:[
					{name:'志愿项目',icon:'project_active.png',badge:5},
					{name:'志愿团队',icon:'team_active.png',badge:2},
					{name:'志愿时长',icon:'time_active.png',badge:1}
...
.header{
	...
	background-color:#ff223f;width:92%;height:30vw;padding:0 4%;display:flex;align-items:center;
	.userinfo{
...
.volunteer_message{
	background-color:#ff223f;width:92%;height:11vw;padding:0 4%;margin-bottom:calc(11vw + 40upx);display:flex;align-items:flex-start;border-radius:0 0 100% 100%;margin-top: -1upx;
	.box{
		...
		.label{
			display:flex;align-items:center;justify-content:center;flex-flow:column;width:100%;height:16vw;color:#666666;font-size:26upx;
			.icon{
				position:relative;width:7vw;height:10vw;margin:0 1vw;
